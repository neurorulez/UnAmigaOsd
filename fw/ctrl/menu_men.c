...
// jepalza, mas FAST RAM admitida que antes
const char *config_memory_fast_msg[] = {"none  ", "2.0 MB", "4.0 MB","8.0 MB","24.0 MB"};
const char *config_cpu_msg[] = {"68000 ", "68010", "-----","68020"}; // jepalza, ahora, es 68020, no alpha
...
// jepalza, arananet añado las opciones AGA exclusivas del MIST
//const char *config_chipset_msg[] = {"OCS-A500", "OCS-A1000", "ECS", "---", "AGA"};
...
//        strcat(s, config_chipset_msg[config.chipset >> 2 & 3]);
    ...
    case MENU_SETTINGS_MEMORY1 :
        ...
        OsdWrite(2, s, menusub == 1,0);
        strcpy(s, "      FAST : ");
        strcat(s, config_memory_fast_msg[config.memory >> 4 & 0x07]);
        OsdWrite(3, s, menusub == 2,0);
    ...
    case MENU_SETTINGS_MEMORY2 :
        if (select)
        {
            ...
            {
                config.memory = ((config.memory + 0x10) & 0x70) | (config.memory & ~0x70);
                if ((config.memory & 0x70) == 0x70)
					config.memory -= 0x70;
